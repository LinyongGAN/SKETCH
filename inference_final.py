...
def evaluate(data_path, model_sft_path, model_train_path, faiss_index_path, seq_len = 128, pred_len = 16):
    ...
    while cnt < 10:
        ...
        coord = data_dict[mmsi][["lat", "lon", "sog", "cog", "coastline_distance", "next_lat", "next_lon"]].values # deg
        ...
        while sog_window.mean()<=18 or sog_window.mean()>=19:
            s = random.randint(0, len(coord)-total_len)
            ...
            print(sog_window.mean(), dist_window.min())
        ...
        print("distance to coastline:", dist_window.min())
        print("vessal average sog: ", sog_window.mean())
        ...
        target_lon_window = target_lon[s:s+total_len, None]
        target_dim = np.concatenate((target_lat_window, target_lon_window), axis=1)
        target_ids = torch.tensor(target_dim[:x_in_len], dtype=torch.float32).unsqueeze(0).to(device)
        true_target = torch.tensor(target_dim[x_in_len:x_in_len+y_out_len], dtype=torch.float32).to(device)
        
        # Generate predictions autoregressively
        predictions = [rad_to_deg(true_out[0, :2].cpu()).numpy()] # 确保初始元素是numpy数组
        current_input = Y_in.clone()  # rad
        
        pred_tgt = None
        for i in tqdm(range(y_out_len)):
            # Prepare model input: [batch_size, seq_len, features]
            input_seq = current_input.clone()  # [1, seq_len, 4]
        
            with torch.no_grad():
                new_position, pred_vol_vec, displacement, pred_coord, pred_target = model(input_seq)
                if pred_tgt is None and pred_target is not None:
        ...
        try:
            ...
            print(f"Distance Loss: {distance}")
